// Isolated service worker for the Gurukul Pedagogy app.
// Scope: /apps/pedagogy/ — its own cache, never shared with other apps.
const CACHE = "maataa-pedagogy-v1";
const SCOPE = "/apps/pedagogy/";
...
self.addEventListener("activate", (event) => {
  ...
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(keys.filter((k) => k.startsWith("maataa-pedagogy-") && k !== CACHE).map((k) => caches.delete(k))),
    ),
  );
  self.clients.claim();
});
