// Isolated service worker for the KBS Knowledge app.
// Scope: /apps/kbs/ — its own cache, never shared with other apps.
const CACHE = "maataa-kbs-v1";
const SCOPE = "/apps/kbs/";
...
self.addEventListener("activate", (event) => {
  ...
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(keys.filter((k) => k.startsWith("maataa-kbs-") && k !== CACHE).map((k) => caches.delete(k))),
    ),
  );
  self.clients.claim();
});
