// Isolated service worker for the Maataa app.
// Scope: /apps/maataa/ — its own cache, never shared with other apps.
const CACHE = "maataa-maataa-v1";
const SCOPE = "/apps/maataa/";
...
self.addEventListener("activate", (event) => {
  ...
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(keys.filter((k) => k.startsWith("maataa-maataa-") && k !== CACHE).map((k) => caches.delete(k))),
    ),
  );
  self.clients.claim();
});
