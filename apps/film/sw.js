// Isolated service worker for the Film app.
// Scope: /apps/film/ — its own cache, never shared with other apps.
const CACHE = "maataa-film-v1";
const SCOPE = "/apps/film/";
...
self.addEventListener("activate", (event) => {
  ...
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(keys.filter((k) => k.startsWith("maataa-film-") && k !== CACHE).map((k) => caches.delete(k))),
    ),
  );
  self.clients.claim();
});
