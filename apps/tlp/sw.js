// Isolated service worker for the TLP app.
// Scope: /apps/tlp/ — its own cache, never shared with other apps.
const CACHE = "maataa-tlp-v1";
const SCOPE = "/apps/tlp/";
...
self.addEventListener("activate", (event) => {
  ...
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(keys.filter((k) => k.startsWith("maataa-tlp-") && k !== CACHE).map((k) => caches.delete(k))),
    ),
  );
  self.clients.claim();
});
