// Isolated service worker for the System app.
// Scope: /apps/system/ — its own cache, never shared with other apps.
const CACHE = "maataa-system-v1";
const SCOPE = "/apps/system/";
...
self.addEventListener("activate", (event) => {
  ...
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(keys.filter((k) => k.startsWith("maataa-system-") && k !== CACHE).map((k) => caches.delete(k))),
    ),
  );
  self.clients.claim();
});
