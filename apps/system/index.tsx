...
function buildRuntimeState(): RuntimeState | RecoveryState {
  ...
  try {
    nativeMetrics = parseNativeMetricFrame(
      createNativeMetricFrame({
        uptimeTicks: 6,
        allocatedMemoryBytes: capsuleBytes,
        activeTasks: SCHEDULER_PROCESSES.length,
        hardwareCores: hostThreadCount(),
        capsuleCount: STATIC_CAPSULES.length,
  ...
  return {
    ...
    metrics: [
      ...
      { label: "Capsules", value: `${nativeMetrics.capsuleCount} loaded`, status: "nominal" },
      { label: "Memory", value: `${nativeMetrics.allocatedMemoryBytes} / 65536 bytes`, status: "nominal" },
      { label: "Scheduler", value: `${nativeMetrics.uptimeTicks} ticks`, status: "nominal" },
    ],
    monitorMetrics: [
      { label: "Runtime", value: "QEMU semihosting", status: "nominal" },
      { label: "Host threads", value: String(nativeMetrics.hardwareCores), status: "nominal" },
      { label: "QEMU engine", value: "netduinoplus2/cortex-m4", status: "nominal" },
      { label: "AI batch", value: `0x${nativeMetrics.aiBatchStatus.toString(16)}`, status: "nominal" },
    ],
    ...
    consoleLines: [
      ...
      { ts: "ipc", level: "debug", text: `native MOSF checksum verified: ${nativeMetrics.checksum.toString(16)}` },
